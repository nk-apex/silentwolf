/**
 * Utils/generics.ts — General-Purpose Utility Functions
 *
 * A grab-bag of small utilities that don't belong to any specific domain.
 * These are used throughout the library.
 *
 * ── KEY EXPORTS ──────────────────────────────────────────────────────────────
 *
 *   BUFFER / JSON:
 *     BufferJSON          — JSON replacer/reviver that round-trips Buffer values
 *                           via base64.  Used by useMultiFileAuthState for safe
 *                           JSON serialisation of binary key material.
 *     toBuffer(stream)    — Drain a readable stream into a single Buffer
 *     unpadRandomMax16(b) — Strip the 1-15 byte random padding added before encryption
 *     encodeBigEndian(n)  — Encode a number as 4-byte big-endian Buffer
 *
 *   IDs:
 *     generateMessageID()   — 16-char lowercase hex message ID
 *     generateMessageIDV2() — UUID v4 message ID (newer WA format)
 *     generateRegistrationId() — Random 16-bit Signal registration ID
 *
 *   TIME:
 *     unixTimestampSeconds()  — Current UNIX timestamp (integer, seconds)
 *     delay(ms)               — Async sleep: await delay(1000) waits 1 second
 *
 *   ERRORS:
 *     getErrorCodeFromStreamError(err) — Extract a DisconnectReason code from a Boom error
 *     getStatusFromReceiptType(type)   — Map receipt type to WA message status enum
 *
 *   EVENTS:
 *     bindWaitForConnectionUpdate(ev)  — Returns a helper to await connection.update events
 *     debouncedTimeout(fn, ms)        — Debounce a function call
 *
 *   VERSION:
 *     fetchLatestBaileysVersion()  — Fetch the latest Baileys-compatible WA version from GitHub
 *     fetchLatestWaWebVersion()    — Fetch the latest WA Web version from WhatsApp's servers
 *
 *   CONTENT:
 *     getKeyAuthor(key)            — Return the JID who authored a WAMessageKey
 *     toNumber(val)                — Convert a Long | number | null to a plain JS number
 *     trimUndefined(obj)           — Remove undefined keys from an object
 *     getCallStatusFromNode(node)  — Parse call status from a BinaryNode
 */

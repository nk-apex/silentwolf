/**
 * Utils/process-message.ts — Secondary Message Processing (processMessage)
 *
 * This file runs AFTER a message has been decrypted by decode-wa-message.ts.
 * It inspects the message content and fires secondary Baileys events based
 * on what the message contains.
 *
 * ── WHAT DOES processMessage() DO? ───────────────────────────────────────────
 *
 * processMessage(message, options) is called for every successfully decrypted
 * WAMessage.  It inspects the message and emits events for:
 *
 *   PROTOCOL MESSAGES (not visible to users in chat):
 *     • HISTORY_SYNC_NOTIFICATION  — Triggers full history download from server
 *     • APP_STATE_SYNC_KEY_SHARE   — Receives app-state decryption keys from
 *                                    another linked device
 *     • REVOKE                     — Message deleted by sender → 'messages.update'
 *     • EPHEMERAL_SETTING          — Disappearing messages timer changed
 *     • CALL_LOG                   — Missed/received call record
 *     • BLOCKS_LID_SENDING         — LID-related block notification
 *
 *   STUB MESSAGES (system events shown in the chat timeline):
 *     • GROUP_PARTICIPANT_ADD      → 'group-participants.update' { action: 'add' }
 *     • GROUP_PARTICIPANT_REMOVE   → 'group-participants.update' { action: 'remove' }
 *     • GROUP_PARTICIPANT_PROMOTE  → 'group-participants.update' { action: 'promote' }
 *     • GROUP_CHANGE_SUBJECT       → 'groups.update' { subject }
 *     • GROUP_CHANGE_DESCRIPTION   → 'groups.update' { desc }
 *     • GROUP_CHANGE_ICON          → 'groups.update' { profilePictureUrl }
 *     • GROUP_CHANGE_INVITE_LINK   → 'groups.update' { revoke }
 *
 *   PRESENCE UPDATES:
 *     • Contact read receipt timestamps → 'chats.update' with unreadCount
 *
 *   HISTORY SYNC:
 *     • Downloads and decrypts history blobs from the media CDN
 *     • Fires 'messaging-history.set' with chats + messages + contacts + isLatest
 *
 * ── RELATIONSHIP TO OTHER FILES ──────────────────────────────────────────────
 *
 *   messages-recv.ts calls processMessage() inside its buffered event zone so
 *   that all secondary events are flushed together with 'messages.upsert'.
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'messaging-history.set'    — Full chat history from a history sync
 *   'chats.update'             — Chat unread count or metadata changed
 *   'chats.delete'             — Chats deleted by a protocol message
 *   'contacts.update'          — Contact display name changed
 *   'messages.update'          — Message revoked / status updated
 *   'messages.delete'          — Messages explicitly deleted
 *   'groups.update'            — Group subject/desc/icon changed
 *   'group-participants.update' — Participant added/removed/promoted/demoted
 *   'labels.edit'              — Label created or updated
 *   'labels.association'       — Label assigned to / removed from a chat
 *   'call'                     — Call log entry created
 */

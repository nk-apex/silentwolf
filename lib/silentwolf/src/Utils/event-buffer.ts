/**
 * Utils/event-buffer.ts — Buffered Event Emitter (makeEventBuffer)
 *
 * This file provides a wrapper around the BaileysEventEmitter that can collect
 * events during a processing batch and flush them all at once — atomically —
 * rather than dispatching them one by one as they are generated.
 *
 * ── WHY IS BUFFERING NEEDED? ─────────────────────────────────────────────────
 *
 * Consider processing a history sync that contains 1 000 messages.  Without
 * buffering, 1 000 separate 'messages.upsert' events would fire, and a bot's
 * event handlers would be called 1 000 times.  With buffering, all 1 000
 * messages are collected into one array and a SINGLE 'messages.upsert' event
 * is emitted containing all of them.
 *
 * ── HOW IT WORKS ─────────────────────────────────────────────────────────────
 *
 *   1. ev.buffer()         — enter buffering mode; events queued, not fired
 *   2. emit many events    — collected in memory
 *   3. ev.flush()          — merge + emit all queued events in one go
 *      ev.flush(false)     — discard queue without emitting (rollback)
 *
 * Events of the same type are merged intelligently:
 *   • 'messages.upsert'   — messages arrays are concatenated
 *   • 'chats.update'      — updates for the same chat ID are merged (deep merge)
 *   • 'contacts.update'   — updates for the same JID are merged
 *   • 'chats.set'         — replaced with the latest value
 *
 * ── USAGE ────────────────────────────────────────────────────────────────────
 *
 * Users don't need to call buffer/flush manually.  It happens automatically
 * inside makeMessagesRecvSocket around each message's processing:
 *
 *   ev.buffer()
 *   try {
 *     await processMessage(...)   // emits many events internally
 *   } finally {
 *     await ev.flush()            // all events arrive in one batch
 *   }
 *
 * ── TRANSACTION HELPER ───────────────────────────────────────────────────────
 *
 *   const safeFn = ev.createBufferedFunction(async () => {
 *     ev.emit('messages.upsert', ...)
 *     ev.emit('chats.update', ...)
 *   })
 *   await safeFn()  // all events are buffered and flushed automatically
 */

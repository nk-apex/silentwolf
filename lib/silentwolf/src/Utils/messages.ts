/**
 * Utils/messages.ts — Message Construction and Normalisation Utilities
 *
 * This is the largest utility file in the library.  It provides:
 *
 * ── MESSAGE CONSTRUCTION ─────────────────────────────────────────────────────
 *
 *   generateWAMessage(jid, content, options) — Build a complete WAMessage from
 *     user-friendly input.  Accepts: text, image, video, audio, document,
 *     sticker, location, contact, reaction, poll, template buttons, lists, etc.
 *
 *   generateWAMessageFromContent(jid, message, options) — Build from a pre-built
 *     proto.IMessage (lower level than generateWAMessage).
 *
 *   generateMessageID()   — Generate a unique 16-char hex message ID
 *   generateMessageIDV2() — Generate a UUID v4 message ID (newer format)
 *
 * ── MESSAGE INSPECTION ───────────────────────────────────────────────────────
 *
 *   getContentType(message)          — Get the top-level content type of a message
 *                                      (e.g. 'imageMessage', 'conversation', etc.)
 *   normalizeMessageContent(message) — Unwrap ephemeral/viewOnce/etc. wrappers
 *   extractMessageContent(message)   — Get the inner-most message (unwrap all wrappers)
 *   getDevice(id)                    — Determine which device sent a message by its ID
 *   getKeyAuthor(key)                — Get the JID who authored a message key
 *
 * ── LINK PREVIEW ─────────────────────────────────────────────────────────────
 *
 *   getUrlInfo(url, options)         — Fetch URL metadata for link previews
 *
 * ── QUOTED MESSAGES ──────────────────────────────────────────────────────────
 *
 *   prepareQuotedField(quoted)       — Build the contextInfo for a quoted reply
 *
 * ── MEDIA MESSAGES ───────────────────────────────────────────────────────────
 *
 *   prepareWAMessageMedia(message, options) — Upload and prepare media content
 *     for attachment to a message (used internally by generateWAMessage)
 *
 * ── CONTENT TYPES REFERENCE ──────────────────────────────────────────────────
 *
 *   Text:        { text: 'Hello!' }
 *   Image:       { image: { url: './photo.jpg' }, caption: 'Caption' }
 *   Video:       { video: { url: './video.mp4' }, caption: 'Caption' }
 *   Audio:       { audio: { url: './audio.mp3' }, mimetype: 'audio/mp4' }
 *   Voice note:  { audio: { url: './voice.ogg' }, ptt: true }
 *   Document:    { document: { url: './file.pdf' }, filename: 'doc.pdf' }
 *   Sticker:     { sticker: { url: './sticker.webp' } }
 *   Location:    { location: { degreesLatitude: -1.28, degreesLongitude: 36.82 } }
 *   Contact:     { contacts: { displayName: 'Alice', contacts: [{ vcard: '...' }] } }
 *   Reaction:    { react: { text: '❤️', key: msg.key } }
 *   Poll:        { poll: { name: 'Fav colour?', values: ['Red', 'Blue'], selectableCount: 1 } }
 *   Forward:     { forward: existingMessage }

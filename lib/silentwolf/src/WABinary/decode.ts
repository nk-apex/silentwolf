/**
 * WABinary/decode.ts — Binary Protocol Decoder
 *
 * WhatsApp uses a custom binary protocol (not JSON, not standard protobuf)
 * to encode stanzas (XML-like tree nodes) over the WebSocket connection.
 * This file decodes incoming binary frames into BinaryNode objects.
 *
 * ── WHATSAPP BINARY PROTOCOL OVERVIEW ────────────────────────────────────────
 *
 * A BinaryNode represents an XML element:
 *   { tag: 'message', attrs: { from: '...', id: '...' }, content: [...children] }
 *
 * The binary encoding is a compact token-based format:
 *   • Common strings (tags, attribute names) are stored as 1-byte token IDs
 *   • Less common strings are stored inline with a length prefix
 *   • JIDs have a compact encoding (user@server split across two tokens)
 *   • Node lists are recursively encoded
 *   • Attribute key-value pairs follow the tag token
 *   • Children follow the attributes
 *
 * The frame wire format:
 *   [1 byte: flags] [3 bytes: big-endian payload length] [N bytes: payload]
 *
 * Flags:
 *   • 0x80 — payload is zlib-deflated (decompress before decoding)
 *   • 0x40 — reserved (ignored by this implementation)
 *
 * ── TOKEN TABLE ──────────────────────────────────────────────────────────────
 *
 * tokens[] in WABinary/constants.ts maps token IDs 0–254 to strings.
 * Two-byte tokens use 0xED (DICTIONARY_0) through 0xF0 (DICTIONARY_3) as a
 * "table select" byte followed by an index byte.
 *
 * ── KEY EXPORTED FUNCTIONS ───────────────────────────────────────────────────
 *
 *   decodeBinaryNode(buffer)
 *     — Decode a complete binary frame into a BinaryNode tree.
 *     — Automatically decompresses zlib frames.
 *     — Returns the root node (usually the outermost stanza).
 *
 *   decodeDecompressed(data)
 *     — Decode already-decompressed bytes (used by noise-handler).
 */

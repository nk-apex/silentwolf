/**
 * Socket/socket.ts — Core WebSocket Layer (makeSocket)
 *
 * This is the INNERMOST layer of the silentwolf middleware chain.  Every other
 * makeXxxSocket function eventually calls this one.  It owns:
 *
 *   • The WebSocket connection to wss://web.whatsapp.com/ws/chat
 *   • The Noise_XX_25519_AESGCM_SHA256 handshake (via makeNoiseHandler)
 *   • The binary framing protocol (3-byte length prefix + BinaryNode encoding)
 *   • QR code generation and display during the first-link flow
 *   • Phone-number pairing code flow (alternative to QR)
 *   • Keep-alive ping/pong (every 30 seconds by default)
 *   • Query / response routing: sendNode(tag) → waitForMessage(tag)
 *   • Connection event emission: 'connection.update' with { connection, qr, lastDisconnect }
 *   • Credential saving triggers: 'creds.update'
 *
 * ── CONNECTION LIFECYCLE ─────────────────────────────────────────────────────
 *
 *   1. WebSocket opens
 *   2. Noise handshake: client sends ephemeral key → server replies → both derive session keys
 *   3. ClientFinish: client sends its identity + registration payload
 *   4. Stream start: receive <stream:features> from server
 *   5. Login or registration:
 *        a. Existing session → sendPassiveIq() → connection 'open'
 *        b. New device      → QR appears → user scans → connection 'open'
 *        c. Pair code       → code shown → user enters in app → connection 'open'
 *   6. keepAlive loop starts (ping every keepAliveIntervalMs)
 *   7. Incoming frames arrive → onFrame() routes them to waiting promises or event handlers
 *   8. On error/close → emit 'connection.update' with lastDisconnect reason
 *   9. Caller (bot code) decides whether to reconnect
 *
 * ── KEY EXPORTS (merged into WASocket) ──────────────────────────────────────
 *
 *   ws                — The raw WebSocket instance
 *   ev                — BaileysEventEmitter (all events flow through this)
 *   authState         — Access to creds + Signal key store
 *   signalRepository  — Signal crypto + LID mapping
 *   user              — Your logged-in JID (populated after connection 'open')
 *   generateMessageTag() — Create a unique message ID
 *   query()           — Send a stanza and await its <iq> response
 *   sendNode()        — Send a BinaryNode over the encrypted connection
 *   logout()          — Gracefully log out and close
 *   end()             — Close the WebSocket without logging out
 *   waitForMessage()  — Await an incoming frame matching a specific tag
 *   sendPresenceUpdate() — Appear online/offline/typing/recording
 *   sendRawMessage()  — Send a WAMessage proto directly
 */

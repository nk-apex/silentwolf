/**
 * Socket/chats.ts — Chat List, Contacts, App-State Sync (makeChatsSocket)
 *
 * This layer manages the chat list, contact list, and WhatsApp's "app state"
 * (the server-side synchronised state that keeps your chat list consistent
 * across all your linked devices).
 *
 * ── WHAT THIS FILE DOES ──────────────────────────────────────────────────────
 *
 *   CHAT MANAGEMENT:
 *     • chatModify() — archive, mute, pin, mark read, delete, clear, star messages
 *     • Stars a message, un-stars it, changes disappearing message timer
 *     • Sends the appropriate app-state mutation to WhatsApp servers
 *
 *   APP-STATE SYNC (appState):
 *     • WhatsApp syncs the chat list via an encrypted "app state" mechanism
 *     • The state is stored as a Merkle-tree of patches on the server
 *     • On connect, fetchAppState() downloads and applies the latest patches
 *     • Patches are verified with HMAC + AES-256-CBC decryption
 *     • Events 'chats.set', 'chats.update', 'contacts.set', 'contacts.update'
 *       are emitted as patches are applied
 *
 *   CONTACT MANAGEMENT:
 *     • fetchStatus(jid)     — get a contact's status message
 *     • fetchBlocklist()     — get your block list
 *     • updateBlockStatus()  — block / unblock a contact
 *     • getPrivacyTokens()   — fetch privacy tokens for groups
 *
 *   PRESENCE:
 *     • presenceSubscribe(jid) — subscribe to a contact's online/typing status
 *     • Incoming <presence> stanzas → 'presence.update' event
 *
 *   MESSAGE PLACEHOLDER (retry):
 *     • Handles placeholder messages (messages that failed to decrypt and need retry)
 *     • Manages the placeholderResendCache for tracking pending resends
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'chats.set'        — Full chat list received (app-state bootstrap)
 *   'chats.upsert'     — New chats
 *   'chats.update'     — Chat metadata changed (mute, archive, pin, etc.)
 *   'chats.delete'     — Chats deleted
 *   'contacts.set'     — Full contact list received
 *   'contacts.upsert'  — New contacts
 *   'contacts.update'  — Contact metadata changed (name, status, etc.)
 *   'presence.update'  — Contact went online/offline/typing/recording
 *   'labels.edit'      — Label created or updated (Business accounts)
 *   'labels.association' — Label assigned to / removed from a chat
 *
 * ── KEY EXPORTED METHODS ─────────────────────────────────────────────────────
 *
 *   chatModify(mod, jid)        — Modify a chat (archive, mute, pin, etc.)
 *   presenceSubscribe(jid)      — Subscribe to contact presence
 *   fetchStatus(jid)            — Fetch contact's status text
 *   fetchBlocklist()            — Get your block list
 *   updateBlockStatus(jid, action) — Block or unblock
 *   fetchPrivacySettings()      — Get your privacy settings

/**
 * Socket/messages-recv.ts — Incoming Message Reception and Processing (makeMessagesRecvSocket)
 *
 * This layer sits just above makeMessagesSendSocket in the middleware chain.
 * It handles EVERYTHING related to receiving messages from WhatsApp:
 *
 * ── WHAT THIS FILE DOES ──────────────────────────────────────────────────────
 *
 *   RECEIPT HANDLING:
 *     • Sends delivery receipts (read, played, sender) back to WhatsApp
 *     • Handles receipt stanzas for our own sent messages (updating their status)
 *
 *   MESSAGE DECRYPTION:
 *     • Processes incoming <message> stanzas via decryptMessageNode()
 *     • Runs decryption inside a Signal key-store transaction for atomicity
 *     • On failure: sends a NACK and schedules a retry request
 *     • On success: fires 'messages.upsert' with the decrypted WAMessage
 *
 *   RETRY MECHANISM:
 *     • Tracks retry counts per message in a NodeCache (1-hour TTL)
 *     • After maxMsgRetryCount failures: stops retrying (message stays as CIPHERTEXT stub)
 *     • Retry request includes the count and our current pre-key bundle
 *
 *   VIEW-ONCE INTERCEPTION (SilentWolf enhancement):
 *     • Detects viewOnceMessage / viewOnceMessageV2 / viewOnceMessageV2Extension wrappers
 *     • Intercepts them BEFORE the 'messages.upsert' event is emitted
 *     • Emits a separate 'messages.view-once' event with full metadata and download helper
 *     • Allows bots to save view-once media that would otherwise be inaccessible
 *
 *   NOTIFICATION PROCESSING:
 *     • Handles <notification> stanzas for group changes, link previews, etc.
 *     • Handles <call> stanzas (missed calls, active calls)
 *
 *   LID RESOLUTION:
 *     • Provides pnToLIDFunc used by LIDMappingStore for USync-based LID lookups
 *     • Sends USyncQuery to resolve PN → LID mappings on demand
 *
 *   HISTORY SYNC:
 *     • Processes initial history sync (chats, messages) from WhatsApp servers
 *     • Fires 'messaging-history.set' and 'chats.set' events
 *
 * ── EVENT FLOW ───────────────────────────────────────────────────────────────
 *
 *   WebSocket frame arrives
 *     → makeSocket.onFrame() routes <message> stanzas here
 *     → decryptMessageNode() decrypts
 *     → processMessage() generates secondary events (group membership changes, etc.)
 *     → ev.emit('messages.upsert', { messages, type: 'notify' })
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'messages.upsert'       — New/updated messages
 *   'messages.update'       — Message status updates (delivered, read, etc.)
 *   'message-receipt.update' — Receipt updates for sent messages
 *   'calls'                 — Incoming call notifications
 *   'chats.update'          — Chat metadata changes
 *   'contacts.update'       — Contact updates (push names)
 *   'groups.update'         — Group metadata changes
 *   'messaging-history.set' — Full history sync result
 */

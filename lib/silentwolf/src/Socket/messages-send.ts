/**
 * Socket/messages-send.ts — Message Sending (makeMessagesSendSocket)
 *
 * This layer sits just above makeSocket in the middleware chain.  It is
 * responsible for ALL outgoing message operations:
 *
 * ── WHAT THIS FILE DOES ──────────────────────────────────────────────────────
 *
 *   MESSAGE CONSTRUCTION:
 *     • Builds the WAMessage proto from user-friendly MessageContent objects
 *     • Applies patchMessageBeforeSending hook (user-configurable transformation)
 *     • Attaches context (quoted message, expiration, viewOnce, mentions, etc.)
 *     • Sets message timestamps and generates message IDs
 *
 *   DEVICE LIST RESOLUTION:
 *     • Fetches the recipient's device list via USyncQuery (pre-keys per device)
 *     • Caches device lists in-memory (5-minute TTL via USER_DEVICES cache)
 *     • Encrypts the message separately for each device (Signal ratchet)
 *     • Fan-out: one sendMessage() call can produce many <enc> nodes (one per device)
 *
 *   ENCRYPTION:
 *     • Uses the Signal repository to encrypt for each recipient device
 *     • pkmsg: first message (uses pre-key bundle from server)
 *     • msg:   subsequent messages (uses existing ratchet session)
 *     • skmsg: group messages (uses the shared sender key)
 *
 *   MEDIA MESSAGES:
 *     • Delegates media upload to messages-media.ts (uploadMedia())
 *     • Attaches the upload result (URL, fileEncSha256, mediaKey, etc.) to the message
 *
 *   RECEIPTS:
 *     • Sends read receipts via sendReadReceipt()
 *     • Sends "played" receipts for voice notes/view-once
 *
 *   PRESENCE:
 *     • sendPresenceUpdate() — online / offline / composing / recording
 *     • Presence is tied to a specific chat JID
 *
 * ── KEY EXPORTED METHODS ─────────────────────────────────────────────────────
 *
 *   sendMessage(jid, content, options)   — Main send function (text, media, reactions, etc.)
 *   sendPresenceUpdate(type, jid)        — Update your presence in a chat
 *   sendReadReceipt(jid, participant, messageIds) — Mark messages as read
 *   sendReceipt(jid, participant, messageIds, type) — Generic receipt sender
 *   relayMessage(jid, message, options) — Low-level: send an already-constructed WAMessage
 *
 * ── USAGE EXAMPLE ────────────────────────────────────────────────────────────
 *
 *   // Send a text message
 ...
 *
 *   // Send an image
 *   await sock.sendMessage(jid, { image: { url: './photo.jpg' }, caption: 'Look!' })
 *
 *   // Send a reaction

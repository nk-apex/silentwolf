/**
 * Socket/communities.ts — WhatsApp Communities (makeCommunitiesSocket)
 *
 * This is the OUTERMOST layer of the middleware chain.  It wraps makeGroupsSocket
 * and adds WhatsApp Communities management.
 *
 * ── WHAT IS A COMMUNITY? ─────────────────────────────────────────────────────
 *
 * A WhatsApp Community is a collection of groups organised under one umbrella.
 * Communities have an announcement channel (the "parent group"), and linked
 * member groups.  Community JIDs end in @g.us like regular groups, but their
 * metadata has the isCommunity flag set.
 *
 * ── KEY EXPORTED METHODS ─────────────────────────────────────────────────────
 *
 *   communityCreate(name, groups)                   — Create a new community
 *   communityLeave(jid)                             — Leave a community
 *   communityUpdate(jid, options)                   — Update name/description/picture
 *   communityLinkGroups(communityJid, groupJids)    — Add groups to a community
 *   communityUnlinkGroups(communityJid, groupJids)  — Remove groups from a community
 *   communityDeactivate(jid)                        — Delete (deactivate) a community
 *   communityJoinApprovalMode(jid, mode)            — Require approval to join
 *   communityMemberAddMode(jid, mode)               — Control who can add members
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'groups.update'             — Community metadata changed

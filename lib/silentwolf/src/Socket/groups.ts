/**
 * Socket/groups.ts — Group Chat Management (makeGroupsSocket)
 *
 * This layer adds all WhatsApp group operations to the socket.
 *
 * ── KEY EXPORTED METHODS ─────────────────────────────────────────────────────
 *
 *   groupCreate(subject, participants)     — Create a new group
 *   groupMetadata(jid)                     — Fetch group info (name, participants, etc.)
 *   groupFetchAllParticipating()           — List all groups you're a member of
 *   groupInviteCode(jid)                   — Get the invite link for a group
 *   groupRevokeInvite(jid)                 — Revoke and regenerate the invite link
 *   groupAcceptInvite(code)                — Join a group via invite code
 *   groupToggleEphemeral(jid, expiration)  — Enable/disable disappearing messages
 *   groupSettingUpdate(jid, setting)       — Change group settings (announce, locked, etc.)
 *   groupMemberAddMode(jid, mode)          — Control who can add members
 ...
 *   groupUpdateDescription(jid, desc)     — Change group description
 *   groupLeave(jid)                        — Leave a group
 *   groupRequestParticipantsList(jid)      — Get pending join requests
 *   groupRequestParticipantsUpdate(jid, participants, action) — Approve/reject requests
 *   groupGetInviteInfo(code)               — Get info about a group from its invite link
 *   groupJoinApprovalMode(jid, mode)       — Require admin approval to join
 *
 * ── GROUP METADATA STRUCTURE ─────────────────────────────────────────────────
 *
 *   { id, subject, creation, owner, desc, participants: [{ id, isAdmin, isSuperAdmin }] }
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'groups.update'        — Group metadata changed
 *   'groups.upsert'        — Added to a new group

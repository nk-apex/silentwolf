/**
 * Socket/newsletter.ts — WhatsApp Newsletters / Channels (makeNewsletterSocket)
 *
 * This layer adds WhatsApp Newsletters (also called "Channels") support.
 * Newsletters are one-way broadcast channels where admins post and subscribers
 * receive.  They use @newsletter JIDs.
 *
 * ── WHAT ARE NEWSLETTERS? ────────────────────────────────────────────────────
 *
 * WhatsApp Newsletters (Channels) allow creators to broadcast messages to
 * large audiences without showing subscriber numbers or identities.
 * They're similar to Telegram Channels.  Newsletters have:
 *   • A unique @newsletter JID (e.g. "254712345678@newsletter")
 *   • Metadata: name, description, picture, subscriber count, role
 *   • Posts that are not E2E encrypted (plaintext on the wire)
 *   • Reactions (subscribers can react to posts)
 *
 * ── KEY EXPORTED METHODS ─────────────────────────────────────────────────────
 *
 *   newsletterCreate(name, desc)                 — Create a new newsletter
 *   newsletterDelete(jid)                        — Delete a newsletter you own
 *   newsletterMetadata(type, key)                — Fetch newsletter metadata
 *   newsletterSubscriptionUpdate(jid, action)    — Subscribe / unsubscribe
 *   newsletterFollow(jid)                        — Follow a newsletter
 *   newsletterUnfollow(jid)                      — Unfollow a newsletter
 *   newsletterMute(jid)                          — Mute notifications
 *   newsletterUnmute(jid)                        — Unmute notifications
 *   newsletterUpdatePicture(jid, content)        — Change newsletter picture
 *   newsletterUpdateName(jid, name)              — Change newsletter name
 *   newsletterUpdateDescription(jid, desc)       — Change description
 *   newsletterReactionMode(jid, mode)            — Enable/disable reactions
 *   newsletterFetchMessages(type, key, count)    — Fetch post history
 *   newsletterToggleMute(jid, mute)              — Mute or unmute
 *
 * ── EVENTS EMITTED ───────────────────────────────────────────────────────────
 *
 *   'messages.upsert' — Newsletter posts (via messages-recv.ts)
 *   'newsletters'     — Newsletter subscription updates
 */
